...
class EscapeValueSetTest extends EasyDBTest
{
    ...
    {
        $cbArgsSets = $this->GoodFactoryCreateArgument2EasyDBProvider();
        $buildArgs = [[['int'], [['1', 2, 3, 5], ['1foo', 2, 3, 5], [null, 2, 3, 5], [true, 2, 3, 5], [false, 2, 3, 5], [new \stdClass(), 2, 3, 5]]], [['string'], [[null, 2, 3, 5], [true, 2, 3, 5], [false, 2, 3, 5], [new \stdClass(), 2, 3, 5]]], [['float', 'decimal', 'number', 'numeric'], [['1foo', 2, 3, 5], [null, 2, 3, 5], [true, 2, 3, 5], [false, 2, 3, 5], [new \stdClass(), 2, 3, 5]]]];
        $args = array_reduce($buildArgs, function (array $was, array $is) {
            foreach ($is[0] as $type) {
                $was = array_merge($was, array_reduce($is[1], function (array $innerWas, array $valueSet) use($type) {
                    $innerWas[] = [$valueSet, $type];
                    return $innerWas;
                }, []));
            }
            return $was;
     ...
     */
    public function testEscapeValueSetThrowsException(callable $cb, array $escapeThis, $escapeThatAsType)
    {
        $db = $this->EasyDBExpectedFromCallable($cb);
        $this->expectException(InvalidArgumentException::class);
     ...
     */
    public function testEscapeValueSet(callable $cb, array $escapeThis, $escapeThatAsType, array $expectOneOfThese)
    {
        $db = $this->EasyDBExpectedFromCallable($cb);
