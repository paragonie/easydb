...
class CellTest extends ColTest
{
    ...
    {
        return [['SELECT 1 AS foo', 0, [], [1]], ['SELECT 1 AS foo, 2 AS bar', 0, [], [1]], ['SELECT 1 AS foo, 2 AS bar', 1, [], [1]], ['SELECT 1 AS foo, 2 AS bar UNION SELECT 3 AS foo, 4 AS bar', 0, [], [1]], ['SELECT 1 AS foo, 2 AS bar UNION SELECT 3 AS foo, 4 AS bar', 1, [], [1]], ['SELECT ? AS foo, ? AS bar UNION SELECT ? AS foo, ? AS bar', 0, [1, 2, 3, 4], [1]], ['SELECT ? AS foo, ? AS bar UNION SELECT ? AS foo, ? AS bar', 1, [1, 2, 3, 4], [1]]];
    }
